...
INDICATORS_DATA = [
    ...
    {'cat': 'Environmental', 'code': 'E.6', 'name': 'المصدر الطبيعي والمتجدد', 'w_global': 0.0506, 'desc': 'نسبة المكونات المتجددة في المادة'},
    # المعايير الاقتصادية
    ...
    {'cat': 'Economic', 'code': 'C.4', 'name': 'تكلفة النقل', 'w_global': 0.0205, 'desc': 'تأثير الوزن والمسافة على تكلفة اللوجستيات'},
...
# قيم الأداء المعيارية (1-9) المقابلة للمجاميع لضمان ظهور واجهة متناسقة غير ممتلئة بالكامل
BM_DISPLAY_SCORES = {
    ...
    with col_nm:
        ...
        for ind in INDICATORS_DATA:
            nm_user_scores[ind['code']] = st.slider(
                f"{ind['code']}: {ind['name']} (NM)", 1.0, 9.0, float(NM_DISPLAY_SCORES[ind['code']]), step=0.001,
                help=ind['desc']
            )
    ...
    with col_bm:
        ...
        for ind in INDICATORS_DATA:
            bm_user_scores[ind['code']] = st.slider(
                f"{ind['code']}: {ind['name']} (BM)", 1.0, 9.0, float(BM_DISPLAY_SCORES[ind['code']]), step=0.001,
                help=ind['desc']
            )
...
with tab2:
    ...
    is_default_nm = all(nm_user_scores[k] == NM_DISPLAY_SCORES[k] for k in NM_DISPLAY_SCORES)
    is_default_bm = all(bm_user_scores[k] == BM_DISPLAY_SCORES[k] for k in BM_DISPLAY_SCORES)
    ...
    fig_radar.add_trace(go.Scatterpolar(
        r=[nm_cat_scores['Environmental'] * 100, nm_cat_scores['Economic'] * 100, nm_cat_scores['Technical'] * 100, nm_cat_scores['Social'] * 100],
        theta=['البيئي', 'الاقتصادي', 'التقني', 'الاجتماعي'], fill='toself', name='الغلاف النانوي (NM)',
        line=dict(color='blue')
    ))
    fig_radar.add_trace(go.Scatterpolar(
        r=[bm_cat_scores['Environmental'] * 100, bm_cat_scores['Economic'] * 100, bm_cat_scores['Technical'] * 100, bm_cat_scores['Social'] * 100],
        theta=['البيئي', 'الاقتصادي', 'التقني', 'الاجتماعي'], fill='toself', name='الغلاف التقليدي (BM)',
        line=dict(color='red')
    ))
    fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 100])), showlegend=True, template="plotly_white")
    ...
    if nm_percentage > bm_percentage:
        st.success(f"القرار معتمد: **الغلاف النانوي (NM) يحقق الأداء الأمثل** ويحقق نسبة تقدم تبلغ **{(nm_percentage - bm_percentage):.3f}%** تحت معايير التصميم الحالية.")
    else:
        st.warning(f"تحذير: **الغلاف التقليدي (BM) يتفوق** بمقدار **{(bm_percentage - nm_percentage):.3f}%**. يرجى إعادة مراجعة نسب ومكونات المواد النانوية.")
