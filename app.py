...
# قيم الأداء المرجعية الافتراضية للبديل التقليدي (BM) المستخرجة والموزونة من مصفوفات جدولك الأخير بدقة
BM_DEFAULT_SCORES = {
...
# قيم الأداء الافتراضية للبديل النانوي (NM) المستخرجة والموزونة من مصفوفات جدولك الأخير بدقة
NM_DEFAULT_SCORES = {
...
with tab2:
    ...
    res_col2.metric("🧱 مؤشر الاستدامة للغلاف التقليدي (BM)", f"{bm_percentage:.2f}%")
    ...
    if nm_percentage > bm_percentage:
        st.success(
            f"القرار معتمد: **الغلاف النانوي (NM) يحقق الأداء الأمثل** ويحقق نسبة تقدم تبلغ **{(nm_percentage - bm_percentage):.2f}%** تحت معايير التصميم الحالية.")
    else:
        st.warning(
